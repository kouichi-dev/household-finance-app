...
この Docker 構成は開発用です。`app` フォルダを `app` / `migrate` の両方にマウントしているため、
コードの変更は `--reload` で自動反映され、マイグレーションを追加した場合も再ビルドせずに反映されます。
（`requirements.txt` や Dockerfile を変更した場合は `--build` が必要です）
...
| on | 日付（例: `2026-08-15`） | その日を含む期間を対象にする。省略時は JST の今日。`2000-01-01`〜`2100-12-31` の範囲外は 422 |
...
```json
{
  "period": { "unit": "monthly", "start": "2026-08-01", "end": "2026-08-31" },
  "prev_on": "2026-07-01",
  "next_on": "2026-09-01",
  "income": 250000,
  "expense": 4200,
...
### 認証はハイブリッド方式
- **アクセストークン**（30分）: JWT の署名で検証する（ステートレス）。トークン用のテーブルは持たず、DB はユーザーの取得にだけ使う
...
### クライアントに日付計算をさせない
一覧と集計のレスポンスは、今の期間（`period`）と「前の期間」「次の期間」の日付（`prev_on` / `next_on`）を含みます。
クライアントはそれを次のリクエストの `on` にそのまま入れるだけで、月送りができます。
`on` を省略すると、サーバーが JST の今日を含む期間を返すので、「今月に戻る」ときもクライアントは日付を知らなくて済みます。
